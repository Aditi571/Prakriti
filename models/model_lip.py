...
dataset = []
target_length = 19683  # Desired length of RGB arrays

for index, row in df.iterrows():
    image_eye_path = '/home/cvlab/Desktop/prakriti/face_masking-main/res/test_res/' + row['AppID'] +'/lip_colors.txt'
...
X_train_scaled = X_train_scaled.reshape(X_train_scaled.shape[0], 243, 243, 1)
X_val_scaled = X_val_scaled.reshape(X_val_scaled.shape[0], 243, 243, 1)

# Create and train the CNN model

def create_cnn_model():
    model = Sequential([
        Conv2D(32, (3, 3), activation='relu', input_shape=(243, 243, 1)),
        MaxPooling2D((2, 2)),
...
cnn_model.fit(X_train_scaled, y_train, epochs=20, batch_size=10, validation_data=(X_val_scaled, y_val))
...
def predict_with_cnn_model(model, X_data):
    ...
    X_data_reshaped = X_data.reshape(X_data.shape[0], 243, 243, 1)
